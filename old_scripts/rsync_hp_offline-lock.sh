...
# Vars
filedir="/storage2/hp_offline_staging"
...
for line in $(awk -F"/" '{print $4}' $varfile); do
	rsync -raPv --ignore-existing --remove-source-files --chmod=u+rwx $filedir/$line rsync://_peer39_app@10.11.1.57/hp_offline/ --password-file /etc/rsync.passwd	
done
echo "" > $varfile
print_log "Finished!"
