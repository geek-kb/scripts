...
# Vars
filedir="/shared/hadoop-test/hpfiles_staging"
varfile="/tmp/varfile"
...
for line in $(awk -F"/" '{print $5}' $varfile); do
	rsync -raPv --ignore-existing --remove-source-files --exclude '.snapshot' --chmod=u+rwx $filedir/$line rsync://_peer39_app@hdedgenldr03.eyedcny.local/hpfiles/ --password-file /etc/rsync.passwd	
done
echo "" > $varfile
print_log "Finished!"
rm -f "$lockfile"
